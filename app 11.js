      ...
      <div class="lesson-path">
        ${unitLessons
          .map((lesson, i) => {
            ...
            return `
          <button class="lesson-node ${built ? "lesson-node-ready" : "lesson-node-locked"}" ${built ? `data-lesson="${lesson.id}"` : "disabled"}>
            <div class="lesson-node-num">${i + 1}</div>
            <div class="lesson-node-body">
              <div class="lesson-node-title">${lesson.title}</div>
              <div class="lesson-node-sub">${built ? lesson.subtitle : "Coming soon"}</div>
            </div>
          </button>`;
          })
          .join("")}
